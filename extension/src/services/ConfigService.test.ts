...
describe("ConfigService", () => {
    ...
    beforeEach(() => {
        ...
        try {
            ...
            const mockContext: MockExtensionContext = {
                globalStorageUri: vscode.Uri.file("/test/global/storage") as unknown as vscodeTypes.Uri,
                subscriptions: [],
            };
            configService = new ConfigService(mockLogger, mockContext as unknown as vscodeTypes.ExtensionContext);
        } catch (e) {
    ...
    test("ConfigService loads derived property correctly", () => {
        ...
        const service = new ConfigService(mockLogger, mockContext as unknown as vscodeTypes.ExtensionContext);    // Assuming a mocked structure where indexing.enabled is derived from a setting
    ...
    test("ConfigService loads default configuration when VS Code config is invalid", () => {
        ...
        const service = new ConfigService(mockLogger, mockContext as unknown as vscodeTypes.ExtensionContext);
    ...
    describe("Global Configuration Storage", () => {
        ...
        test("loadQdrantConfig falls back to global if local missing", async () => {
            ...
            // Mock Global Read
            const config = { qdrant_config: { url: "http://global" }, ollama_config: { base_url: "http://global" } };
            ...
            expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining("global storage"), "CONFIG");
        });
    });
    ...
    describe("Detailed Connection Validation", () => {
        test("returns partial failure if only one service is down", async () => {
            const config: QdrantOllamaConfig = {
                ...
                ollama_config: { base_url: "http://ollama", model: "test-model" }, // Added model for completeness
            };

            // Mock Ollama Success
            (global.fetch as Mock).mockImplementation((url: string) => {
                if (url.includes("ollama")) return Promise.resolve({ ok: true });
                if (url.includes("qdrant")) return Promise.resolve({ ok: false, statusText: "Not Found" });
    ...
    test("ConfigService.dispose cleans up resources", () => {
        ...
        expect((service as unknown as { _listeners: unknown[] })._listeners).toEqual([]);
    });
    ...
    test("ConfigService returns immutable config copies", () => {
        ...
        (config1 as unknown as { indexing: { enabled: boolean } }).indexing.enabled = false;
    ...
    test("ConfigService.loadQdrantConfig handles URLs with leading slashes correctly", async () => {
        ...
        const result = await configService.loadQdrantConfig(mockFolder as vscodeTypes.WorkspaceFolder);
    ...
    describe("saveQdrantConfig", () => {
        test("saveQdrantConfig creates directory and writes file", async () => {
            ...
            await configService.saveQdrantConfig(mockFolder as unknown as vscodeTypes.WorkspaceFolder, newConfig as unknown as QdrantOllamaConfig);
        ...
        test("saveQdrantConfig handles write errors", async () => {
            const mockFolder: Partial<vscodeTypes.WorkspaceFolder> = { uri: { fsPath: "/test/workspace" } as vscodeTypes.Uri };
            const config = {
            ...
            } as Partial<QdrantOllamaConfig>;

            vi.mocked(vscode.workspace.fs.stat).mockResolvedValue({ type: vscode.FileType.Directory } as Partial<vscodeTypes.FileStat>); // Dir exists
            vi.mocked(vscode.workspace.fs.writeFile).mockRejectedValue(
                new Error("Permission denied")
            );

            await expect(
                configService.saveQdrantConfig(mockFolder as unknown as vscodeTypes.WorkspaceFolder, config as unknown as QdrantOllamaConfig)
