import { ActionAttributes, ParsedAction } from "../webviews/protocol.js";
...
export class XmlParser {
  ...
  parseSingle(rawXml: string, index: number): ParsedAction | null {
    ...
    if (!match) {
      console.error(
        "XML Parser failed to identify command type or opening tag:",
        rawXml
      );
      return null;
    }
    ...
    const parsedAction: ParsedAction = {
      ...
      rawXml,
      ...attributes,
      // Default multiLineApprove to false if not specified
      multiLineApprove: attributes.multiLineApprove ?? false,
    };
    ...
    switch (type) {
      ...
      case "read": {
        ...
        // Only set content if it's not empty (for self-closing tags)
        const trimmedContent = fullContent.trim();
        if (trimmedContent) {
          parsedAction.content = trimmedContent;
        }
        break;
      }
      ...
      case "file": {
        ...
        const searchMatch = fullContent.match(
          new RegExp(`<search>([\\s\\S]*?)</search>`, "i")
        );
        const replaceMatch = fullContent.match(
          new RegExp(`<replace>([\\s\\S]*?)</replace>`, "i")
        );
        ...
        if (searchMatch && replaceMatch) {
          ...
          // Simple file creation/overwrite (only if content exists)
    ...
    if (parsedAction.type === "file" && !parsedAction.path) {
      ...
      parsedAction.errorDetails =
        'Missing required "path" attribute for file action.';
    }
