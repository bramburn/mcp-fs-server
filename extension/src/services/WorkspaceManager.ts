...
import { GitProvider, GitRepository, VsCodeGitProvider, FallbackGitProvider } from '../git/GitProvider.js';
...
export class WorkspaceManager implements vscode.Disposable {
    ...
    constructor(
        ...
        private readonly _configService: ConfigService, // Add ConfigService dependency
    ...
    private createGitProvider(): GitProvider {
        try {
            ...
            console.warn('VS Code Git provider not available, using fallback:', error);
    ...
    private handleWorkspaceFoldersChanged(e: vscode.WorkspaceFoldersChangeEvent): void {
        ...
        console.log(`Workspace folders changed: Added ${e.added.length}, Removed ${e.removed.length}`);
        
        // Update repositories
        this.findRepositories().then(() => {
            // Notify listeners after repositories are updated
            this.notifyListeners(event);
        }).catch(error => {
            console.error('Error updating repositories after workspace change:', error);
        });
    }
    ...
    public async findRepositories(): Promise<string[]> {
        try {
            ...
            console.log(`Found ${repoPaths.length} Git repositories:`, repoPaths);
    ...
    public async getRepository(path: string): Promise<GitRepository | null> {
        try {
            ...
            console.error('Error getting repository for path:', path, error);
            return null;
        }
    }
    ...
    public async isGitRepository(path: string): Promise<boolean> {
        try {
            ...
            console.error('Error checking if path is Git repository:', path, error);
            return false;
        }
    }
    ...
    public getActiveWorkspaceFolder(): vscode.WorkspaceFolder | undefined {
        ...
        for (const folder of folders) {
            if (this._repositories.some(repo => 
     ...
     */
    public getRepositoriesForFolder(folder: vscode.WorkspaceFolder): GitRepository[] {
        return this._repositories.filter(repo => 
            repo.rootPath.startsWith(folder.uri.fsPath) || 
            folder.uri.fsPath.startsWith(repo.rootPath)
        );
    }
    ...
    public async checkWorkspaceIntegrity(): Promise<void> {
        ...
        if (folders.length > 1) {
            ...
            console.log(`Multi-root workspace: ${folders.length} folders, ${configCount} configurations, ${repoCount} repositories`);
        }
    }
     ...
     */
    public removeWorkspaceChangeListener(listener: WorkspaceChangeListener): void {
